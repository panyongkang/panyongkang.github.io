...
这两个东西经常被混淆，因为Nacos一个产品把两个功能都做了。但本质上它们是不同的：

| 对比维度           | **注册中心 (Service Registry)**                | **配置中心 (Config Center)**             |
| :----------------- | :--------------------------------------------------- | :--------------------------------------------- |
| **核心职责** | 管理**服务实例的地址**，让服务A能找到服务B在哪 | 管理**配置文件**，让所有服务统一拉取配置 |
| **存储内容** | IP + 端口 + 服务名 + 健康状态                        | 数据库连接、开关配置、业务参数等               |
| **典型问题** | “订单服务挂了，用户服务怎么知道？”                 | “改了数据库密码，难道要重启100个服务？”      |
| **数据特点** | 动态变化（服务随时上下线）                           | 相对稳定，但需要热更新                         |
| **类比**     | 114查号台（查号码）                                  | 公司公告栏（贴公告）                           |

### Eureka / ZooKeeper / Nacos 区别

| 组件                | 类型       | 核心特点                              | 当前状态                   |
| :------------------ | :--------- | :------------------------------------ | :------------------------- |
| **Eureka**    | 注册中心   | AP（可用性优先），简单但功能单一      | ❌ 已停更，老项目在用      |
| **ZooKeeper** | 分布式协调 | CP（一致性优先），可做注册/配置，但重 | ✅ 活跃，适合分布式锁/选主 |
| **Nacos**     | 注册+配置  | **AP/CP可切换**，二合一，功能强 | ✅ 主流首选，新项目用这个  |
| **Apollo**    | 纯配置中心 | 配置功能比Nacos更强                   | ✅ 适合复杂配置管理        |
...
ecip.voucher.0002.11 5001200004106	电子印章打印查询凭证信息(柜面)  QUERYSEALINFO
/bank/business/无纸化和电子印章/ElecSealUtil/QueryVcherHeight.lfc

--服务路由信息表 登记渠道请求的服务路由到指定交易
select t.*,t.rowid from TP_CIP_SERVICERTADM t where t.p_servicecode in ('ecip.voucher.0001.17','ecip.voucher.0001.02','5001200004106')
insert into TP_CIP_SERVICERTADM (CHANNELCODE, P_SERVICECODE, MODULECODE, TRANSCODE, ROUTERSTATUS, CALLMTH, ROUTESEQNO, MACCHECK, ROWID)
values ('*', 'ecip.voucher.0001.02', 'PL', 'CREATEVOU', '1', '', null, '0', 'AAAU60AAFAAAAG1AAE');

insert into TP_CIP_SERVICERTADM (CHANNELCODE, P_SERVICECODE, MODULECODE, TRANSCODE, ROUTERSTATUS, CALLMTH, ROUTESEQNO, MACCHECK, ROWID)
values ('*', '5001200004106', 'PL', 'QUERYSEALINFO', '1', '', null, '0', 'AAAU60AAFAAAAG2AAG');

insert into TP_CIP_SERVICERTADM (CHANNELCODE, P_SERVICECODE, MODULECODE, TRANSCODE, ROUTERSTATUS, CALLMTH, ROUTESEQNO, MACCHECK, ROWID)
values ('*', 'ecip.voucher.0001.17', 'PL', 'ADDSEALTOPDF', '1', '', null, '0', 'AAAU60AAFAAAAG2AAL');
```
...
###### XQ-DS-2024-0156_ST_Key_01 优化客户预留信息录入规则的需求

```SQL
--990803-行政区域代码管理交易，在测试"同步"功能前先执行以下sql：
create table IB_PARA_COMBOITEM_INFO_SYNBAK as select * from IB_PARA_COMBOITEM_INFO 
where ENTRYNAME in ('combo_行政区划（1级）_XZQH','combo_行政区划（2级）_XZQH','combo_行政区划（3级）_XZQH');

--树递归SQL平铺数据处理：
select decode(level,1,'combo_行政区划（1级）_XZQH',2,'combo_行政区划（2级）_XZQH',3,'combo_行政区划（3级）_XZQH','combo_行政区划（其他）_XZQH') entryname,t.areacode itemprefix,decode(t.areacode,110100,'北京市',120100,'天津市',310100,'上海市',500100,'重庆市',t.areadel) itemvalue,decode(t.areacode,520000,'0',to_char(rownum+100)) orderid from T_CORE_AREA t start with t.supareacode='999999999' connect by prior t.areacode=t.supareacode union all select decode(mod(rownum,2),0,'combo_行政区划（2级）_XZQH',1,'combo_行政区划（3级）_XZQH','combo_行政区划（其他）_XZQH') entryname,a.areacode itemprefix,a.areadel itemvalue,to_char(rownum + 10000) orderid from (select areacode, areadel,rownum from T_CORE_AREA,(select rownum from dual connect by level<3) where areacode in ('710000','810000' ,'820000' )) a union all select decode(mod(rownum,2),0,'combo_行政区划（2级）_XZQH',1,'combo_行政区划（3级）_XZQH','combo_行政区划（其他）_XZQH') entryname,b.areacode itemprefix,b.areadel itemvalue,to_char(rownum + 10000) orderid from (select areacode, areadel,rownum from T_CORE_AREA,(select rownum from dual connect by level<3) where areacode in ('990000')) b
```

```
初始化系统参数组件：获取tp_cip_sysparameters-系统参数信息表配置参数
--系统参数信息表
select * from tp_cip_sysparameters where TRANSCODE='CoreAreaSyn' and MODULECODE = 'IB'
insert into tp_cip_sysparameters (MODULECODE, TRANSCODE, PARAMKEYNAME, PARAMLEVEL, PARAMVALUE, PARAMDESC, MANAGEFLAG, INITFLAG)
values ('IB', 'CoreAreaSyn', 'CONF_FILEPATH', '2', '/apphome/ctbsafa/afa/conf/tft.conf', 'CONF文件路径', '1', '1');

insert into tp_cip_sysparameters (MODULECODE, TRANSCODE, PARAMKEYNAME, PARAMLEVEL, PARAMVALUE, PARAMDESC, MANAGEFLAG, INITFLAG)
values ('IB', 'CoreAreaSyn', 'DELSQL', '2', 'DELETE FROM T_CORE_AREA', '数据删除语句', '1', '1');

insert into tp_cip_sysparameters (MODULECODE, TRANSCODE, PARAMKEYNAME, PARAMLEVEL, PARAMVALUE, PARAMDESC, MANAGEFLAG, INITFLAG)
values ('IB', 'CoreAreaSyn', 'LOCAL_FILEPATH', '2', '/upload_files/tell_sync', '文件下载成功服务器路径', '1', '1');

insert into tp_cip_sysparameters (MODULECODE, TRANSCODE, PARAMKEYNAME, PARAMLEVEL, PARAMVALUE, PARAMDESC, MANAGEFLAG, INITFLAG)
values ('IB', 'CoreAreaSyn', 'TABLENAME', '2', 'T_CORE_AREA', '数据同步表名', '1', '1');

IB_COREAREASYN.req	核心行政区域同步	0	/reppth	F		TextString	100	报表路径	Y	/reppth	AAAU5lAAIAAAcMcAAA

/FCBank/trade/Trade/DP/PM/t990803/Button_同步_OnClick.lfc
核心行政区域同步 IB_COREAREASYN CoreAreaSyn

[15:27:38.917][I]Step2_Node15 贵阳银行文件TFT上传下载
[15:27:38.917][I]入参0:="PB/20260722/00002456_xzqycode.txt"
[15:27:38.917][I]入参1:="PB/20260722/00002456_xzqycode.txt"
[15:27:38.917][I]入参2:=0
[15:27:38.917][I]入参3:=""
[15:27:38.917][I]入参4:="/home/ctbsafa/afa/conf/tft.conf"
[15:27:38.917][I]入参5:="down"
[15:27:38.917][I]入参6:="/upload_files/tell_sync"

afa服务器文件路径：
/home/ctbsafa/afa/upload_files/tell_sync/PB/20260722/00002456_xzqycode.txt

AdmCode行政区划分代码110000北京市
AdmCode行政区划分代码110100市辖区110000
AdmCode行政区划分代码110101东城区110100
AdmCode行政区划分代码110102西城区110100
AdmCode行政区划分代码110105朝阳区110100
AdmCode行政区划分代码110106丰台区110100
```
